...
<head><meta http-equiv="Content-Type" content="text/html; charset=utf-8">
  ...
  <title>FIXWEI - Editar Comunicaciones</title>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>Editar Comunicaciones</h1>
          </div>
          
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="home">Inicio</a></li>
              <li class="breadcrumb-item active">Editar Comunicaciones</li>
            </ol>
          </div>
        </div>
        <div>
        ...
        <div class="row">
            ...
            <div class="col-12">
              ...
              <form role="form" action="controlador/comunicacionInterna/controllerComunicacionInterna" method="POST">
                 <?php
                 $id=$_POST['id'];
                 $consultarPermisosComunicaciones=$mysqli->query("SELECT * FROM comunicaciones WHERE id='$id' ");
                 $extraeDatos=$consultarPermisosComunicaciones->fetch_array(MYSQLI_ASSOC);
                 
                 if($extraeDatos['tipo'] == 'cargo'){
                     $activaCargo='checked';
                 }
                 if($extraeDatos['tipo'] == 'usuario'){
                     $activaUsuario='checked';
                 }
                 if($extraeDatos['tipo'] == 'grupo'){
                    $activaGrupo='checked'; 
                 }
                 ?>
                <input name="id" value="<?php echo $extraeDatos['id'] ?>" type="hidden" readonly>
                <div class="card-body">
                    <div class="row">
                        <div class="form-group col-md-6">
                            <label>Agregar quienes tienen permiso para publicar: </label><br>
                               <input type="radio" id="rad_cargoA2" name="radiobtnP2" value="cargo" <?php echo $activaCargo; ?> >
                                <label for="cargo">Cargo</label>
                                <input type="radio" id="rad_usuarioA2" name="radiobtnP2" value="usuario" <?php echo $activaUsuario; ?> >
                                <label for="usuarios">Usuarios</label>
                                <input type="radio" id="rad_grupo" name="radiobtnP2" value="grupo" <?php echo $activaGrupo; ?> >
                                <label for="usuarios">Grupos</label>
    
                                
                                <div class="select2-blue">
                                    <select class="select2" multiple="multiple" data-placeholder="Seleccionar encargado" style="width: 100%;" name="select_encargadoA2[]" id="select_encargadoA2">
                                        <?php
                                    if($extraeDatos['tipo'] == 'cargo'){
                                        $acentos2 = $mysqli->query("SET NAMES 'utf8'");
                                        $cargoAsociados =  json_decode($extraeDatos['activos']);
                                        /// END
                                    ...
                                    if($extraeDatos['tipo'] == 'usuario'){
                                        ...
                                        $cargoAsociados =  json_decode($extraeDatos['activos']);
                                        /// END
                                        $consultaCargos=$mysqli->query("SELECT * FROM usuario ORDER BY nombres ");
                                        while ($columna = $consultaCargos->fetch_array()) { 
                                    ...
                                    if($extraeDatos['tipo'] == 'grupo'){
                                        ...
                                        $cargoAsociados =  json_decode($extraeDatos['activos']);
                                        /// END
                                        $consultaCargos=$mysqli->query("SELECT * FROM grupo ORDER BY nombre ");
                                        while ($columna = $consultaCargos->fetch_array()) { 
                                    ...
                                    ?>
                                    <option value="<?php echo $columna['id']; ?>" <?php echo $seleccionarCt; ?>  ><?php echo $columna['nombre']; ?> </option>
                                    <?php 
                                        }
                                    }
                                ?>
                ...
                <div class="card-footer">
                    <button type="submit" name="publicidadActivaEditar" class="btn btn-primary float-right">Actualizar</button>
                </div>
                </div>
                </form>
                <!-- /.card-body -->
