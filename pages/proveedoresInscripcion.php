...
<!DOCTYPE html>
<html>
    <title>Solicitar inscripción y aprobación de proveedor</title>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>Solicitar inscripción y aprobación de proveedor</h1>
          </div>
          
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="home">Inicio</a></li>
              <li class="breadcrumb-item active">Solicitar inscripción y aprobación de proveedor</li>
            </ol>
          </div>
        </div>
        <div>
            ...
            <div class="row">
            <div class="col-sm">
                <button type="button" class="btn btn-block btn-info btn-sm"><a href="agregarProveedor"><font color="white"><i class="fas fa-plus-square"></i> Nueva solicitud</font></a></button>
            </div>
            <div class="col-sm">
            ...
            <div class="col-sm">
                <button type="button" class="btn btn-block btn-info btn-sm"><a href="agregarProveedoresTipo"><font color="white"><i class="fas fa-user-plus"></i> Tipo de proveedor</font></a></button>
            </div>
            <div class="col-sm">
                <button type="button" class="btn btn-block btn-info btn-sm"><a href="agregarProveedoresCriticidad"><font color="white"><i class="fas fa-user-plus"></i> Criticidad</font></a></button>
            </div>
            <div class="col-sm">
                <form action="exportacion/proveedores" method="post"> <!-- Onclick="window.location='exportacion/proveedores'" -->
                <input name="inscripcion" value="1" type="hidden">
            ...
            <div class="col-sm">
                   <button type="button" class="btn btn-block btn-info btn-sm"><a href="proveedores" ><font color="white"><i class="fas fa-list"></i> Proveedores aprobados</font></a></button>
            </div>
                ...
                <table class="table table-head-fixed text-center" id="example">
                  <thead>
                    <tr>
                      ...
                      <!--<th>Meses para evaluaci&oacute;n</th> style="display:<?php //echo $visibleI;?>;"-->
                      <th >Documentos</th>
                  ...
                  <tbody>
                    ...
                    $data = $mysqli->query("SELECT * FROM proveedores WHERE estado='Pendiente' OR estado='rechazado'  ORDER BY razonSocial ASC")or die(mysqli_error());
                     ...
                     while($row = $data->fetch_assoc()){
                        ...
                        $quienElaboraConteo = $row['radio']; 
                        $quienElaboraIDconteo = json_decode($row['aprobador']);
                        
                        if($quienElaboraConteo == "cargo" && $row['notificacion'] == 'Pendiente'){
                            if(in_array($cargo,$quienElaboraIDconteo)){
                                $permisoVerMasConteo = TRUE;
                            }
                        }
                        
                        if($quienElaboraConteo == "usuario" && $row['notificacion'] == 'Pendiente'){
            	    ...
            	    echo" <td>".$row['telefono']."</td>";
                        ...
                        $fecha_entrada = strtotime($fechaUltimaRevision);
                        
                        if($fecha_actual > $fecha_entrada){
                           //$mysqli->query("UPDATE proveedores SET estado='Pendiente' WHERE id='$id' ");
                           $mensajeDelProveedor='<font color="red">Documentos vencidos '.$fechaRevisar.'</font>';
                           $saleVencimiento='0';
                        }else{
                           $saleVencimiento='1';
                           $mensajeDelProveedor=$fechaRevisar; 
                    	    ...
                    	    if($row['estado'] == 'Pendiente'){
                    	        ...
                    	        $validandoEstadosCarpetas=$mysqli->query("SELECT * FROM proveedores WHERE id='$id' ");
                    	        $extraearValidandoEstadosCarpetas=$validandoEstadosCarpetas->fetch_array(MYSQLI_ASSOC);
                    	          
                    	        
                    	        
                    	        if($extraearValidandoEstadosCarpetas['notificacion'] == 'Rechazado'){
                    	              echo" <td><font color='red'>Rechazado</font></td>";
                    	        }else{
                    	        
                        	        if($saleVencimiento == '1'){
                        	            echo" <td><font color='blue'>".$row['estado']."</font></td>";
                        	        }else{
                        	            echo" <td><font color='green'>".$row['estado']."</font></td>";
                        	        }
                    	        }
                    	    }
                    	    if($row['estado'] == 'Rechazado'){
                    	            echo" <td><font color='red'>".$row['estado']."</font></td>";
                    	    }
            	    ...
            	    //echo "<td>".$mensajeDelProveedor."</td>";
            	      
            	    //echo" <td>".$row['tiempoEvaluacion']."</td>"; style='display:$visibleI;'
                    
                    ///// vamos a enviar unos parametros para identificar si existe una carpeta raiz con el id existente del proveedor
                    /*  //// enlace anterior y obsoleto
                   
                    echo"<form action='proveedorDocumetosCarpetas' method='POST'>";
                    echo"<input type='hidden' name='idProveedor' value= '$id' >";
                    echo"<button type='submit' class='btn btn-block btn-primary btn-sm'><i class='fas fa-file-upload'></i> Documentos</button>";
                    echo"</form>";
                    */
                     echo "<td>";
                    echo"<form action='proveedorDocumetosCarpetasB' method='POST'>";
                    echo"<input type='hidden' name='idProveedor' value= '$id' >";
                    echo"<button type='submit' class='btn btn-block btn-primary btn-sm'><i class='fas fa-file-upload'></i> Documentos</button>";
                    echo"</form>";
                    echo "</td>";
                    ...
                    echo"<input type='hidden' name='idProveedor' value= '$id' >";
                    echo"<td style='display:$visibleI;'><button type='submit' class='btn btn-block btn-warning btn-sm'><i class='fas fa-cubes'></i> Productos</button></td>";
                    echo"</form>";
                    
                    */
                    
                     echo"<form action='proveedoresVerInscripcion' method='POST'>";
                    echo"<input type='hidden' name='idProveedor' value= '$id' >";
...
<script type="text/javascript">
  $(function() {
    ...
    ?>
        Toast.fire({
            type: 'warning',
            title: ' Los documentos del proveedor fueron rechazados.'
        })
    <?php
    }
