...
$allowedFileType = ['application/vnd.ms-excel','text/xls','text/xlsx','application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'];
         ...
         $last = end($data);
           ...
           $contador++;
                    ...
                    if($cedula == ""){
                        ...
                        if(is_numeric($cedula)){
                            ...
                            if($numNom > 0){//si el nombre está repetido se pone falso
                                ...
                                $tring_compararCedula.=$tipoEnviarAlerta.' '.$cedula.', ';
                ...
                if(isset($Row[4])) {
                     ...
                     $enviamos_celda_numeracion=$contador_caracteres_fecha++;
                     $enviamos_celda_numeracion_b=$contador_caracteres_fecha_b++;
                    ...
                    if($fecha == ""){
                        ...
                        /// validamos la cantidad de caracteres permitidos ha entrar
                           ...
                           '<br>';
                        if(strlen($fecha) > 10 || strlen($fecha) < 10){  
                            $alertaFechaPermitida=FALSE;
                            
                            
                            if(is_numeric($fecha)){
                                echo 'aca A: '.$fecha_string_no_permitido=$fecha;   
                            }else{
                                if(strlen($fecha) < 10){
                                 echo $fecha_string_no_permitido=$fecha;
                                }else{
                                echo 'aca A: '.$fecha_string_no_permitido='existen caracteres especiales en la celda '.($contador_caracteres_fecha);
                            ...
                            if($posicion_coincidencia_fecha === false){
                               ...
                               /*
                                echo 'cadena: '.$cadena_carecteres=["|","°","!",'"',"#","$","%","&","/","(",")","'"];
                                $coincidencia_caracteres_contador='0';
                                for($bc=0; $bc<count($cadena_carecteres); $bc++){
                                    echo 'imprimir: '.$cadena_carecteres[$bc]; 
                                    echo $cadena_carecteres_fecha = $cadena_carecteres[$bc];
                                    echo ' - '.$coincidencia_caracteres= strpos($fecha, $cadena_carecteres_fecha);
                                    if($coincidencia_caracteres != NULL){
                                        echo ' - contando: '.$coincidencia_caracteres_contador='1';
                                    }
                                    echo '<br>';
                                }
                                
                                echo 'sale info: '.$coincidencia_caracteres_contador;
                                echo '<br>';
                               */
                                
                                if(is_numeric($fecha)){
                                echo ' A: '.$fecha_string_no_permitido=$fecha;   
                                }else{
                                echo     'aca A: '.$fecha_string_no_permitido='existen caracteres especiales en la celda '.($contador_caracteres_fecha_b);
                                }
                                
                            }else{ 
                            
                                //// reemplazamos el punto por el guion
                                 'Fecha else :'.$fecha=str_replace(".","-",$fecha);
                                 '<br>';
                                ...
                                if(is_numeric($diaValidadoEntranteValidar)){
                                    if($diaValidadoEntranteValidar >= 01 && $diaValidadoEntranteValidar <= 31){
                                        ...
                                        $alertaFechaPermitida=FALSE;
                                          ...
                                          '3 NO solo numerico';
                                         $fecha_string_no_permitido.='('.$fecha.') '; // agregamos cuando invierten la fecha y se van muchos 0000, ejemplo 12-03-2000, esa fecha está mal porque entiende los días como años   
                                }
                                /// año
                                 '<br>';
                                 ...
                                 if($diaValidado > 0 && $diaValidado < 10){
                                   ...
                                   'fecha v: '.$fechaValidadoUsuario=$resultadoFechaEdad.'-'.$mesValidado.'-'.$enviarCero.''.$diaValidado; 
                                  $resultadoFechaEdad.'-12-31';    
                                   '-- Fecha: '.$fecha;
                                   '<br>';
                                 
                                 
                                 if($fecha > $fechaValidadoUsuario || $enviarDatoAlerta == 1){  
                                           '<font color="red">(</font>';
                               ...
                               $tamanoArray = count($separar);
                                ...
                                for($m =0; $m <= $tamanoArray; $m++){
                                    ...
                                    if($separar[$m] == ""){
                                         ...
                                         $separar[$m];  '<br>';
                                        $validacionGrupo = $con->query("SELECT * FROM grupo WHERE nombre = '$separar[$m]'")or die(mysqli_error($con));
                                        ...
                                        if($extraerNombreValidarGrupoDistri['nombre'] != NULL){
                                             ...
                                             ' Grupos de distribución: '.$m.' '.$separar[$m].' -- <font color = "RED">Este grupo de distribución no existe en el sistema.</font> ';
                                             '<br>';
                                             $string_grupos.=$separar[$m].', ';
                                        }
            ...
            <form id="formularioFallaFecha" action="../../usuarios" method="POST" onsubmit="procesar(this.action);" >
                <input name="mensajeAlertaFallo" value="1" type="hidden">
        				...
        				if($validarArchivo < 0 || $validarArchivo > 0){ // evitamos subir un archivo diferente
                           ...
                           if($alertaFechaPermitida == FALSE || $repiteNombreCedulaExiste == FALSE || $repiteGrupos == FALSE || $repiteCargoAsociado == FALSE || $validarNumericoTT == FALSE || $correoConfirmado == FALSE || $alertaEdad == FALSE || $validarNumericoA == FALSE || $tipoDocumento == FALSE || $cedulaValidar == FALSE || $cargoValidar == FALSE || $liderValidar == FALSE || $procesoValidar == FALSE || $centroTrabajoValidar == FALSE || $centroGrupoValidar == FALSE || $repiteNombreCedula == FALSE || $campoNull == FALSE){ //$centroCostoValidar == FALSE || 
                             // echo 'Validaciones';
                                ...
                                $activacion_existente_usuario=1;
                                }
                               ...
                               'ante del registro: '.$contadorFecha;
                                 ...
                                 //echo 'No permite guardar';
                                  //echo 'alerta edad<br>';
                                    ...
                                    <form name="miformularioAlertaedadEnviar" action="../../usuarios" method="POST" onsubmit="procesar(this.action);" >
                                        ...
                                        if($string != NULL){
                                        ?>
                                        <input name="variableFechaB" value="<?php echo $string; ?>" type="hidden">
                                        <?php
                                        }elseif($stringB != NULL){
                                        ?>
                                        <input name="variableFechaB" value="<?php echo $stringB; ?>" type="hidden">
                                        <?php
                                        }elseif($stringC != NULL){
                                        ?>
                                        <input name="variableFechaB" value="<?php echo $stringC; ?>" type="hidden">
                                        <?php
                                        }else{
                                        ?>
                                    ...
                                    <?php
                                            ...
                                            echo '<br><br><br>Registrar'.$fecha.'<br>'; 
                                            /*
  ...
  { 
    ...
    if($redireccionAlertaFechaPermitid == 1){ //echo 'alerta fecha<br>';
        ?>
                                    ...
                                    <form name="miformularioAlertaedadFechaPermitida" action="../../usuarios" method="POST" onsubmit="procesar(this.action);" >
                                        ...
                                        <input name="mensajeFechaNoPermitido" value="<?php echo $fecha_string_no_permitido;?>" type="">
                                    </form> 
                            <?php
    }
    ...
    if($activacion_existente_usuario == '1'){
                                    ?>
                                            <script> 
                                                 window.onload=function(){
                                            ...
                                            <form name="miformularioRepiteExiste" action="../../usuarios" method="POST" onsubmit="procesar(this.action);" >
                                                ...
                                                <input type="hidden" name="mensaje_existente_usuario" value="<?php echo $tring_compararCedula;?>">
                                            </form> 
                                    <?php    
    }
