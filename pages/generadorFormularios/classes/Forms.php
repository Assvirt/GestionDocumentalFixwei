...
Class Forms extends DBConnection{
    ...
    public function save_form(){
        ...
        if(empty($form_code)){
            $save_form = $this->conn->query("INSERT INTO `form_list` set $data ")or die(mysql_error());
        }else{
            $save_form = $this->conn->query("UPDATE form_list SET $data where form_code = '$form_code' ")or die(mysql_error());
        }
        if($save_form){
            $resp['status'] = 'success';
        }else{
    ...
    public function save_response(){
        ...
        $rl_insert = $this->conn->query("INSERT INTO response_list set $data ")or die(mysql_error());
        if($rl_insert){
        ...
        $save_resp = $this->conn->query("INSERT INTO responses (rl_id,meta_field,meta_value) VALUES $data")or die(mysql_error());
        if($save_resp){
            $resp['status'] = 'success';
        }else{
            $resp['status'] = 'failed';
