...
if(isset($_POST['AgregarCentro'])){
 ...
 // funcion para quitar espacios
        ...
        $salida = Quitar_Espacios($nombreCentro);
        ...
        $salidaB = Quitar_EspaciosB($prefijoCentro);
    
    ///////// consultamos la tabla y extraemos el nombre
    $validacion = $mysqli->query("SELECT * FROM centrodetrabajo WHERE  nombreCentrodeTrabajo LIKE '%$salida%' OR prefijoCentrodeTrabajo LIKE '%$salidaB%' ")or die (mysqli_error());
  ...
  /////////////// End validamos el nombre que entra al nombre de la consulta
    ...
    $validacion1 = $mysqli->query("SELECT * FROM centrodetrabajo WHERE nombreCentrodeTrabajo = '$salida' AND id_centrodetrabajo != '$id'");//consulta a base de datos si el nombre se repite
    $numNom = mysqli_num_rows($validacion1);
    if($numNom > 0){//si el nombre está repetido se pone falso
        $editar = false;
        //echo '<script language="javascript">alert("El nombre del centro de trabajo ya existe.");</script>';
        ?>
            <script> 
                 window.onload=function(){
    ...
    $validacion2 = $mysqli->query("SELECT * FROM centrodetrabajo WHERE prefijoCentrodeTrabajo ='$salidaB'  AND id_centrodetrabajo != '$id'");//consulta a base de datos si el nombre se repite
    $numNom2 = mysqli_num_rows($validacion2);
    if($numNom2 > 0){ //echo 'si el prefijo está repetido se pone falso';
        $editar2 = false;
        //echo '<script language="javascript">alert("El prefijo del centro de trabajo ya existe.");</script>';
        ?>
            <script> 
                 window.onload=function(){
    ...
    //echo '<script language="javascript">window.location.href="../../centrodetrabajo"</script>';
    ?>
            <script> 
                 window.onload=function(){
