                                                ...
                                                if(file_exists('../../archivos/solicitudes')){
                                                    if(move_uploaded_file($guardado, '../../archivos/solicitudes/'.$archivoNombre)){
                                                        ...
                                                        ?>
                                                            ...
                                                            <form name="miformulario" aaction="../../crearDocumentoMasivo" method="POST" onsubmit="procesar(this.action);" >
...
if(isset($_POST['ejecutador'])){
    ...
    /// traemos todos los documentos que están en vigentes
    $recorrido_documento_obsoleto=$mysqli->query("SELECT * FROM `documento` order by id DESC ");  //WHERE vigente='1' AND pre='si' 
    while($extraer_recorrido_documento=$recorrido_documento_obsoleto->fetch_array()){
        ...
        if($contadorVigente > 0 || $contadorObsoleto > 0){  // echo 'primera alerta';
            $activoPrimeraAlerta=1;
        }else{
        ...
        $validarUpdateA=$mysqli->query("UPDATE documento SET pre = NULL, codificacion='$codificacion', consecutivo='$consecutivo', version='$version' WHERE pre='si' AND id='".$extraer_recorrido_documento['id']."' ");
        $validarUpdateB=$mysqli->query("UPDATE solicitudDocumentos SET estado ='Ejecutado' WHERE id='".$_POST['idSolicitudDocumento']."' ");
        
            if($validarUpdateA != NULL && $validarUpdateB != NULL){
                 'guarda';
                $enviarVariableGuardado=1;
                        
            }else{ 
                 'valida';
                $validaVariableAlerta=1;
    ...
    if($activoPrimeraAlerta == 1){
         'primera alerta';
        
                    ?>
                        <script> 
                             window.onload=function(){
                               document.forms["miformularioAlertaPrimera"].submit();
                             }
                        </script>
                                                             
                        <form name="miformularioAlertaPrimera" action="../../crearDocumentoMasivo" method="POST" onsubmit="procesar(this.action);" >
    ...
    if($enviarVariableGuardado == 1){
        ...
        ?>
                            ...
                            <form name="miformularioRegistro" action="../../crearDocumentoMasivo" method="POST" onsubmit="procesar(this.action);" >
                                <input name="validacionAgregarD" value="1" type="hidden">
                            </form> 
                        <?php
    }
    
    if($validaVariableAlerta == 1){
        //echo 'valida';
        ...
        ?>
                            <script> 
                                 window.onload=function(){
                                    ...
                                    document.forms["miformularioSegundaAlerta"].submit();
                                 }
                            </script>
                        <form name="miformularioSegundaAlerta" action="../../crearDocumentoMasivo" method="POST" onsubmit="procesar(this.action);" >
                            <input name="alertaConsecutivo" value="1" type="hidden">
                        </form> 
            <?php
    }                
    ...
    $recorrido_documento_obsoleto_verificando=$mysqli->query("SELECT * FROM `documento` WHERE vigente='1' AND pre='si' order by id DESC ");
    ...
    ?>
                        <script> 
                             window.onload=function(){
                                ...
                                document.forms["miformularioDecisionAlerta"].submit();
                             }
                        </script>
                         
                        <form name="miformularioDecisionAlerta" action="../../crearDocumentoMasivo" method="POST" onsubmit="procesar(this.action);" >
                            <?php 
...
if(isset($_POST['ejecutadorIndividual'])){
    ...
    if($contadorVigente > 0 || $contadorObsoleto > 0){  'activa validación';
        ...
        $mysqli->query("UPDATE solicitudDocumentos SET estado ='Ejecutado' WHERE id='".$_POST['idSolicitudDocumento']."' ");
                    ?>
                        ...
                        <form name="miformularioAprobado" action="../../crearDocumentoMasivo" method="POST" onsubmit="procesar(this.action);" >
                            <input name="validacionAgregarD" value="1" type="hidden">
                        </form> 
                    <?php
    }
