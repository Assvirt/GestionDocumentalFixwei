    ...
    $prioridad = $_POST['prioridad'];
     ...
     // funcion para quitar espacios
        function Quitar_Espacios($nivelCargo)
        {
          ...
          return  trim($nivelCargo);
        }
        /// END
        $nivelCargo = Quitar_Espacios($nivelCargo);
        ...
        function Quitar_EspaciosB($prioridad)
        {
          ...
          return  trim($prioridad);
        }
        /// END
        $prioridad = Quitar_EspaciosB($prioridad);
    
    ///////// consultamos la tabla y extraemos el nombre
    $validacion = $mysqli->query("SELECT * FROM nivelcargo WHERE nivelCargo = '$nivelCargo' OR prioridad = '$prioridad'")or die (mysqli_error());
...
if($numRows > 0){
   ...
   $editar2 = true;
    ...
    $validacion1 = $mysqli->query("SELECT * FROM nivelcargo WHERE nivelCargo = '$nivelCargo'  AND id != '$id'");//consulta a base de datos si el nombre se repite
    ...
    if($editar != false){
          $mysqli->query("UPDATE nivelcargo SET  nivelCargo='$nivelCargo'  WHERE id = '$id'")or die(mysqli_error($mysqli));
    }
    
    
    $validacion2 = $mysqli->query("SELECT * FROM nivelcargo WHERE prioridad = '$prioridad'  AND id != '$id'");//consulta a base de datos si el nombre se repite
